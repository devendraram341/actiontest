 ...
 *
 */
public class UpdateRoleForUserBean implements Processor {

	private static final Logger logger = LoggerFactory.getLogger(UpdateRoleForUserBean.class);
...
//				JSONObject userRoleData = new JSONObject(message.getBody(String.class).trim());
			...
			try {
				service.updateRoleForUser(user, roleAlreadyAssociated, roleToInherit, tenant);
			} catch (Exception exp) {
