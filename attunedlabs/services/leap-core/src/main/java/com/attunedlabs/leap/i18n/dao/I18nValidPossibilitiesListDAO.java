...
public class I18nValidPossibilitiesListDAO {
	...
	public int insertNewVpList(final String tenantId, final String siteId, final String feature,
		...
		String methodName = "insertNewVpList";
		logger.debug("{} entered into the method {} tenantId: {},  siteId:{}  vpListI18nId:{} vpType:{} feature:{}",
				LEAP_LOG_KEY, methodName, tenantId, siteId, vpListI18nId, vpType, feature);
			...
			DefaultUpdateSummary insertSummary = (DefaultUpdateSummary) dataContext.executeUpdate(new UpdateScript() {
				...
				public void run(UpdateCallback callback) {
					...
					insert.value(LeapI18nConstant.TENANT_ID, tenantId).value(LeapI18nConstant.SITE_ID, siteId)
							.value(LeapI18nConstant.FEATURENAME, feature)
							.value(LeapI18nConstant.VPLISTI18NID, vpListI18nId).value(LeapI18nConstant.VPTYPE, vpType);
					insert.execute();
	...
	public List<LeapValidPossibilitiesList> selectAllTenantVpList(String tenantId, String siteId, String feature)
			throws LocaleResolverException {
		String methodName = "selectAllTenantVpList";
	...
	public LeapValidPossibilitiesList selectVpList(String tenantId, String siteId, String feature, String vpType)
			throws LocaleResolverException {
		String methodName = "selectVpList";
		logger.debug("{} entered into the method {} tenantId: {},  siteId:{}  vpType:{} feature:{}", LEAP_LOG_KEY,
				methodName, tenantId, siteId, vpType, feature);
		LeapValidPossibilitiesList messageContext = null;
		DataSet dataSet = null;
		Connection connection = null;
		try {
	...
	public List<LeapValidPossibilitiesList> selectAllVpList() throws LocaleResolverException {
		logger.debug("{} inside selectAllMessage() ...", LEAP_LOG_KEY);
		List<LeapValidPossibilitiesList> listMessageContext = new ArrayList<>();
		LeapValidPossibilitiesList messageContext = null;
		DataSet dataSet = null;
		Connection connection = null;
	...
	/*
	 * public static void main(String[] args) { I18nValidPossibilitiesListDAO text =
	 * new I18nValidPossibilitiesListDAO(); try { //
	 * text.insertNewVpList("Fidelity", "wh1", "Parcel",
	 * "Parcel.ParcelMaintFrm.isInterNational", "Radio"); //
	 * List<LeapValidPossibilitiesList> list = text.selectAllVpList(); //
	 * List<LeapValidPossibilitiesList> list =
	 * text.selectAllTenantVpList("Fidelity", "wh1", "Parcel"); //
	 * logger.debug("list : "+list); LeapValidPossibilitiesList textValue =
	 * text.selectVpList("Fidelity", "wh1", "Parcel", "Radio");
	 * logger.debug("textValue : "+textValue); } catch (LocaleResolverException e) {
	 * e.printStackTrace(); } }
	 */
}
