...
public class EmailPipeLineUtil {
	...
	/**
	 * method to set the exchange body in leapHeader and set the body node from the
	 * exchange and update the exchange with its value
	 * 
	 * @param exchange
	...
	public void setExchangeBodyToLeapServiceContext(Exchange exchange) throws EmailNotifierException, IOException {
		...
		MailAttachments mailAttachments = pipeActivity.getEmailNotifyActivity().getEmailNotification()
				.getMailAttachments();
		LOGGER.debug("{} XmlInput available:{} ", LEAP_LOG_KEY, xmlInput);
	...
	/**
	 * 
	 * Method to attach file in mails which will be called only when attachment is
	 ...
	 * @throws EmailNotifierException
	 */
	public void getExchangeBodyFromLeapServiceContext(Exchange exchange) throws EmailNotifierException {
