...
public class I18nValidPossibilitiesDAO {
	 ...
	 */
	public List<String> selectTenantValidPossibilities(String tenantId, String siteId, String feature, String vpType,
			String vpListI18nId, String localeId) throws LocaleResolverException {
		String methodName = "selectTenantValidPossibilities";
		...
		try {
			...
			Query q = dataContext.query().from(table1).innerJoin(table2).on(vpListId_1, vpListId_2).selectAll()
					.where(LeapI18nConstant.TENANT_ID).eq(tenantId).and(LeapI18nConstant.SITE_ID).eq(siteId)
					.and(LeapI18nConstant.FEATURENAME).eq(feature).and(LeapI18nConstant.VPTYPE).eq(vpType)
					.and(LeapI18nConstant.VPLISTI18NID).eq(vpListI18nId).and(LeapI18nConstant.LOCALE_ID).eq(localeId)
					.toQuery();
	 ...
	 * @throws LocaleResolverException
	 */
	public List<LeapValidPossibilities> selectAllTenantLocaleSpecificValidPossibilities(String tenantId, String siteId,
			String loacaleId) throws LocaleResolverException {
		String methodName = "selectAllTenantLocaleSpecificValidPossibilities";
		logger.debug("{} entered into the method {}",LEAP_LOG_KEY, methodName);
		...
		try {
			...
			Query q = dataContext.query().from(table1).innerJoin(table2).on(vpListId_1, vpListId_2).selectAll()
					.where(LeapI18nConstant.TENANT_ID).eq(tenantId).and(LeapI18nConstant.SITE_ID).eq(siteId)
					.and(LeapI18nConstant.LOCALE_ID).eq(loacaleId).toQuery();
			dataSet = dataContext.executeQuery(q);
			if (dataSet == null)
				throw new LocaleResolverException("Empty dataSet returned on get all available messages! ");
			Iterator<Row> itr = dataSet.iterator();
	...
	public List<LeapValidPossibilities> selectAllValidPossibilities() throws LocaleResolverException {
		String methodName = "selectAllValidPossibilities";
		logger.debug("{} entered into the method {}",LEAP_LOG_KEY, methodName);
		List<LeapValidPossibilities> listMessageContext = new ArrayList<>();
		LeapValidPossibilities messageContext = null;
		DataSet dataSet = null;
	 ...
	 * @return
	 */
	private LeapValidPossibilities parseROW(LeapValidPossibilities messageContext, Row row, Table table1,
			Table table2) {
