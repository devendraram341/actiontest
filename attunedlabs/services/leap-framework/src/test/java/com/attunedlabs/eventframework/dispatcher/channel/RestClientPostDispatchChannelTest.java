...
public class RestClientPostDispatchChannelTest {

	final Logger log = LoggerFactory.getLogger(RestClientPostDispatchChannelTest.class);
	...
	public void loadConfigurations() throws EventFrameworkConfigParserException {
		if (eventFrameworkConfig == null)
			eventFrameworkConfig = EventAndDispatchFileRead.getRestDispatch();
		if(systemEvent==null)
	...
	/**
	 * this method used for check Rest DispatcherChannel.
	 * 
	 * @throws DispatchChannelInitializationException
	 * @throws MessageDispatchingException
	 */
	@Test
	public void testRestClientPostDispatchChannel()
		...
		List<DispatchChannel> eventList = eventFrameworkConfig.getDispatchChannels().getDispatchChannel();
		Assert.assertTrue("DispatchChannel list should not be empty or zero (0) :: ", eventList.size() > 0);
		DispatchChannel restDis = null;
		for (DispatchChannel event : eventList) {
			if (event.getId().equalsIgnoreCase("REST_CLIENT")) {
				restDis = event;
				break;
			}
		}
		Assert.assertNotNull("Dispatch Channel Value should be exist ::", restDis);
		RestClientPostDispatchChannel restClientPostDispatchChannel = new RestClientPostDispatchChannel(
				restDis.getChannelConfiguration());
		restClientPostDispatchChannel.dispatchMsg(testData, requestContext, "PRINT_SERVICE");
	}
		...
		for (EventDispatcher eventDispacher : eventDispacherList) {
			...
			if (transformationtype.equalsIgnoreCase("XML-XSLT")) {
				...
				BufferedReader reader = new BufferedReader(
						new InputStreamReader(RestClientPostDispatchChannelTest.class.getClassLoader().getResourceAsStream(xslName)));
