...
public class KafkaQueueDispatchChannelTest {

	private final String jsonConfigStr = "{\"queueName\":\"testQueue\"}";
	final Logger logger = LoggerFactory.getLogger(KafkaQueueDispatchChannelTest.class);
	...
	public void init() throws EventFrameworkConfigParserException, ConfigPersistenceException {
		if (eventFrameworkConfig == null)
			eventFrameworkConfig = EventAndDispatchFileRead.getKafkaQueue();
		if (perService == null)
	...
	/**
	 * this method used for check Kafka queue DispatcherChannel.
	 * 
	 ...
	 */
	@Test
	public void testAKafkaQueueDispatchChannel() throws DispatchChannelInitializationException, InterruptedException,
		...
		for (DispatchChannel disChannel : disChannelList) {
			if (disChannel.getId().equalsIgnoreCase("KAFKA_QUEUE")) {
		...
		KafkaQueueDispatchChannel hcDisChannel = new KafkaQueueDispatchChannel(
	...
	public void testBConsumer() {
		...
		consumer.subscribe(Collections.singletonList("testQueue"));
		...
		while (true) {
			...
			for (ConsumerRecord<String, String> record : messageConsumed) {
				...
				Assert.assertEquals("{\"queueName\":\"testQueue\"}", record.value());
			}
			if (flag) {
				break;
			}
	...
	private DispatchChannel getDispatchChannelWithFileStore() {
		...
		for (DispatchChannel event : disChannel) {
			if (event.getId().equalsIgnoreCase("KAFKA_QUEUE")) {
				fileStoreDis = event;
				break;
			}
		}
