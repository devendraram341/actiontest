...
public class HazelcastTopicDispatchChannelTest {

	final Logger log = LoggerFactory.getLogger(HazelcastTopicDispatchChannelTest.class);
	...
	public void loadConfigurations() throws EventFrameworkConfigParserException {
		if (eventFrameworkConfig == null)
			eventFrameworkConfig = EventAndDispatchFileRead.getHazelCastTopic();
		if(systemEvent==null)
	...
	/**
	 * 
	 * this method used for check HazelCase Topic DispatcherChannel.
	 * 
	 * @throws DispatchChannelInitializationException
	 * @throws MessageDispatchingException
	 */
	@Test
	public void testHazelcastTopicDispatchChannel()
		...
		DispatchChannel hzDis = null;
		for (DispatchChannel event : eventList) {
			if (event.getId().equalsIgnoreCase("HAZELCAST_TOPIC")) {
				hzDis = event;
				break;
			}
		}
		Assert.assertNotNull("Dispatch Channel Value should be exist ::", hzDis);
		HazelcastTopicDispatchChannel hazelcastTopicDispatchChannel = new HazelcastTopicDispatchChannel(
				hzDis.getChannelConfiguration());
		hazelcastTopicDispatchChannel.dispatchMsg(testData, requestContext, "PRINT_SERVICE");
	...
	public void testDispatchforEventWithRetryStatusTrue()
		...
		for (Event event : eventList) {
			if (event.getId().equalsIgnoreCase("PRINT_SERVICE_JSON_HZ")) {
				evt = event;
				break;
			}
		}
	...
	public void testDispatchforEventWithXSLT() throws EventFrameworkConfigurationException,
		...
		Event evt = null;
		for (Event event : eventList) {
			if (event.getId().equalsIgnoreCase("PRINT_SERVICE_XSLT_HZ")) {
				evt = event;
				break;
			}
		}
	...
	public void testDispatchforEventWithCustom() throws EventFrameworkConfigurationException,
		...
		Event evt = null;
		for (Event event : eventList) {
			if (event.getId().equalsIgnoreCase("PRINT_SERVICE_CUSTOM_HZ")) {
				evt = event;
				break;
			}
		}
