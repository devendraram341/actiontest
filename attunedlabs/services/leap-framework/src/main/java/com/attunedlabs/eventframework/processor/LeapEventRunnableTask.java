...
public class LeapEventRunnableTask implements Runnable, Serializable {
	...
	public void run() {
		logger.debug("{} RoiEvent= {}", LEAP_LOG_KEY, leapEvt);

		/*
		 * try{ List <IEventProcessor> procList=getEventProcessors();
		 * for(IEventProcessor evtProcIns:procList){ logger.info(" Processor is-"+
		 * evtProcIns.getClass()); evtProcIns.processEvent(leapEvt); } } catch(Exception
		 * exp){ exp.printStackTrace(); }
		 */

		// once processor completed post to Dispatcher
		// IEventFrameworkIDispatcherService dispatcherService=new
		// EventFrameworkDispatcherService();
