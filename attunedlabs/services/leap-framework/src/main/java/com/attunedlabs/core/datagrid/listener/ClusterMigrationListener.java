...
public class ClusterMigrationListener implements MigrationListener {
	...
	public void migrationStarted(MigrationEvent migrationEvent) {
		logger.debug("{} Started: {}", LEAP_LOG_KEY, migrationEvent);
	}

	@Override
	public void migrationCompleted(MigrationEvent migrationEvent) {
		logger.debug("{} Completed:********* {} ***********", LEAP_LOG_KEY, migrationEvent);
		...
		try {
			...
			DataSet dataSet = dataContext.query().from(table).select(EventTrackerTableConstants.EVENT_STORE_ID)
					.execute();
			Iterator<Row> itr = dataSet.iterator();
			if (itr.hasNext()) {
				Row row = itr.next();
				eventStoreKey = row.getValue(table.getColumnByName(EventTrackerTableConstants.EVENT_STORE_ID))
						.toString();
				...
				logger.debug("{} migrated eventStoreKey :{} with eventMap size :{} ", LEAP_LOG_KEY, eventStoreKey,
						eventList.size());
			} else {
				logger.debug("{} EventDispatcherTracker table is empty no events!", LEAP_LOG_KEY);
			}
		} catch (Exception e) {
			logger.warn("{} cannot get migration status for hazelcast event map db operation failed!", LEAP_LOG_KEY);
		} finally {
	...
	public void migrationFailed(MigrationEvent migrationEvent) {
		logger.debug("{} Failed: {}", LEAP_LOG_KEY, migrationEvent);
	}
}