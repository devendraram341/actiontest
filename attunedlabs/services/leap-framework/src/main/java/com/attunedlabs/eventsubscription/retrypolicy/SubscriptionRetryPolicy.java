...
public class SubscriptionRetryPolicy {
	...
	/**
	 * retry RetryIntervalMultiplier will increase the interval gap by the number
	 * RetryIntervalMultiplier is specified.</br>
	 * (default RetryIntervalMultiplier = 2 e.g:- 1000 * 2 * RetryInterval= 2000
	 * next time double of the result)</br>
	 * <b>Applicable for processingStatus : NEW, PROCESS, RETRY_INPROCESS, , RETRY_,
	 * only.</b>
	 */
	 ...
	 * first retry will be done at this interval if property RetryInterval is not
	 * specified.</br>
	 * <b>Applicable for processingStatus : NEW, PROCESS, RETRY_INPROCESS, , RETRY_,
	 * only.</b>
	 */
	 ...
	 * MaximumRetryInterval (default MaximumRetryInterval = 1min) this period will
	 * be mostly used when not specified.</br>
	 * <b>Applicable for processingStatus : NEW, PROCESS, RETRY_INPROCESS, , RETRY_,
	 * only.</b>
	 */
	...
	/**
	 * time unit will be considered for retry list all the computations will be done
	 * based on the timeUnit.</br>
	 * <b>Applicable for processingStatus : NEW, PROCESS, RETRY_INPROCESS, , RETRY_,
	 * only.</b>
	 */
	...
	/**
	 * Setting the RetryCount to a negative value such as -1 will then always retry
	 * (unlimited). Setting the maximumRetries to 0 will not retry at all.</br>
	 * <b>Applicable for processingStatus : NEW, PROCESS, RETRY_INPROCESS, , RETRY_,
	 * only.</b>
	 */
	...
	/**
	 * default queue name which means records with records will be sent to this jms
	 * queue on exception.
	 */
	protected static String defaultRetryQueue = "retry_all_all";

	/**
	 * default value is 15 which means records with particular status will be
	 * retried until the max records are not mentioned.
	 */
	protected static int defaultRetryQueueConsumers = 4;
	 ...
	 */
	public static String getRetryQueueName(JSONObject retryConfigurationJSON) {
		if (getPropertyFor(RETRY_QUEUE_NAME, retryConfigurationJSON) != null)
			if (getPropertyFor(RETRY_QUEUE_NAME, retryConfigurationJSON) > 0)
				return getPropertyFor(RETRY_QUEUE_NAME, retryConfigurationJSON).toString().toUpperCase().trim();
		retryConfigurationJSON.put(RETRY_QUEUE_NAME, defaultRetryQueue);
		return defaultRetryQueue;
	}

	/**
	 * @return the retryConsumers
	 */
	public static int getRetryConsumers(JSONObject retryConfigurationJSON) {
		if (getPropertyFor(RETRY_CONSUMERS, retryConfigurationJSON) != null)
			if (getPropertyFor(RETRY_CONSUMERS, retryConfigurationJSON) > 0)
				return getPropertyFor(RETRY_CONSUMERS, retryConfigurationJSON);
		retryConfigurationJSON.put(RETRY_CONSUMERS, defaultRetryQueueConsumers);
		return defaultRetryQueueConsumers;
	}
	...
	private static Integer getPropertyFor(String key, JSONObject retryConfigurationJSON) {
		try {
			...
			logger.warn("{} to parse value for key: {} should be Integer..!", LEAP_LOG_KEY, key);
		}
		return null;
	}
