...
public class PipelineExecutionInitializer {
	...
	public void decidePipelineRoute(Exchange exchange) throws Exception {
		String methodName = "decidePipelineRoute";
	...
	public void processPipelineRequest(Exchange exchange) throws PipeLineExecutionException {
		String methodName = "processPipelineRequest";
		logger.debug("{} entered into the method {}", LEAP_LOG_KEY, methodName);
		boolean isavalidXml = PipelineExecutionUtil.checkForXmlContent(exchange);
		String body = exchange.getIn().getBody(String.class);
		PipelineExecution execution = exchange.getIn().getHeader(PipelineServiceConstant.PIPELINE_EXECUTION,
	...
	public void processPipelineConfigRequest(Exchange exchange) throws Exception {
		String methodName = "processPipelineConfigRequest";
		logger.debug("{} entered into the method {}", LEAP_LOG_KEY, methodName);
		boolean isavalidXml = PipelineExecutionUtil.checkForXmlContent(exchange);
		String body = exchange.getIn().getBody(String.class);
		PipelineExecution execution = exchange.getIn().getHeader(PipelineServiceConstant.PIPELINE_EXECUTION,
		...
		if (doc != null) {
			String integerationPipesStr = PipelineExecutionUtil
					.convertDocumentToString(execution.getIntegerationPipesDocument());
