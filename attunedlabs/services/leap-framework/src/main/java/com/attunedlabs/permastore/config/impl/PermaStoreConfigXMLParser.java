...
public class PermaStoreConfigXMLParser {
	...
	private void validateXml(String configXMLFile) throws PermaStoreConfigParserException {
		...
		try {
			...
			logger.trace(".validateXml() Custom Error Handler while Validating XML against XSD");
			SchemaFactory factory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
			Schema schema = factory
					.newSchema(PermaStoreConfigXMLParser.class.getClassLoader().getResource(SCHEMA_NAME));
			...
			logger.error("{} Perma Store Config XML Schema Validation Failed for file ", LEAP_LOG_KEY, exp);
			throw new PermaStoreConfigParserException("Permastore Config XML Schema Validation Failed for file ", exp);
		}
		logger.debug("{} exiting from the {}", LEAP_LOG_KEY, methodName);
	}

	public PermaStoreConfigurations marshallConfigXMLtoObject(String configXMLFile)
		...
		try {
			...
			PermaStoreConfigurations configurations = (PermaStoreConfigurations) jaxbUnmarshaller
					.unmarshal(inputSourceConfigXml);

			return configurations;

		} catch (JAXBException exp) {
			logger.error("{} PermaStoreConfig XMLParsing Failed for file ", LEAP_LOG_KEY, exp);
	...
	public PermaStoreConfigurations marshallXMLtoObject(String psConfigxml) throws PermaStoreConfigParserException {
		try {
			...
			JAXBContext jaxbContext = JAXBContext
					.newInstance(com.attunedlabs.permastore.config.jaxb.ObjectFactory.class);
			Unmarshaller jaxbUnmarshaller = jaxbContext.createUnmarshaller();
			// PermaStoreConfiguration
			// configuration=(PermaStoreConfiguration)jaxbUnmarshaller.unmarshal(new
			// StringReader(psConfigxml));
			PermaStoreConfigurations configurations = (PermaStoreConfigurations) jaxbUnmarshaller
					.unmarshal(new StringReader(psConfigxml));
	...
	public String unmarshallObjecttoXML(PermaStoreConfiguration permaStoreConfiguration)
			throws PermaStoreConfigParserException {
		try {
			...
			logger.debug("{} xmlObjecttoXML()= {}", LEAP_LOG_KEY, theXML);
			return theXML;
