...
public class PolicyConfigurationService extends GenericApplicableNode implements IPolicyConfigurationService {
	...
	public void addPolicyConfiguration(ConfigurationContext configurationContext, Policy policyConfig)
		...
		String methodName = "addPolicyConfiguration";
		logger.debug("{} entered into the method {}, tenant={} , site={}, For Policy={} ", LEAP_LOG_KEY, methodName,
				configurationContext.getTenantId(), configurationContext.getSiteId(), policyConfig.getPolicyName());
		...
		try {
			...
			logger.error("{} Failed to add Policy= {} ",LEAP_LOG_KEY, policyName, e);
	...
	public PolicyConfigurationUnit getPolicyConfigurationUnit(PolicyRequestContext polRequestContext, String policyName)
			throws PolicyConfigurationException {
		String methodName = "getPolicyConfigurationUnit";
		logger.debug("{} entered into the method {}, PolicyName={}, RequestContext={} ", LEAP_LOG_KEY, methodName,policyName,polRequestContext);
		try {
			...
			logger.error("{} .getPolicyConfigurationUnit() Invalid Config Node Tree",LEAP_LOG_KEY, e);
			throw new PolicyConfigurationException("Invalid Config Node Tree", e);

		} catch (ConfigServerInitializationException e) {
			logger.error("{} .getPolicyConfigurationUnit() Failed to connect to Config Server",LEAP_LOG_KEY, e);
	...
	public Object getPolicyResponseData(PolicyRequestContext policyRequestContext, String policyName)
		...
		String methodName = "getPolicyConfigurationUnit";
		logger.debug("{} entered into the method {}, PolicyName={}, RequestContext={} ", LEAP_LOG_KEY, methodName,policyName,policyRequestContext);
		PolicyConfigurationUnit configUnit;
		try {
			configUnit = getPolicyConfigurationUnit(policyRequestContext, policyName);
			logger.trace("{} evaluation list : {} " ,LEAP_LOG_KEY, configUnit.getEvaluationUnitList());
			if (configUnit == null)
			...
			if (bol) {
				Object policyResponseData = reqHandler.getPolicyResponseData(configUnit, policyRequestContext);
				logger.debug("{} exiting from the {}", LEAP_LOG_KEY, methodName);
			...
			logger.error("{} Failed to retreive Policy {} ",LEAP_LOG_KEY, policyName, e);
	...
	private void loadConfigurationInDataGrid(PolicyConfigurationUnit polConfigUnit)
			throws PolicyConfigurationException {
		logger.trace("{} .loadConfigurationInDataGrid() PolicyConfigurationUnit=" ,LEAP_LOG_KEY, polConfigUnit);
		try {
			LeapConfigurationServer configServer = LeapConfigurationServer.getConfigurationService();
	...
	public boolean changePolicyStatus(ConfigurationContext configurationContext, String policyName, boolean isEnable)
		...
		String methodName = "changePolicyStatus";
		...
		try {
			...
			// Disable Request
			if (!isEnable) {
				// We have to Disable policyConfig hence remove from DataGrid and update DB as
	...
	public boolean deletePolicy(ConfigurationContext configurationContext, String policyName)
			throws PolicyConfigurationException {
		String methodName = "deletePolicy";
		logger.debug("{} entered into the method {}, configurationContext={}, policyName={} ", LEAP_LOG_KEY, methodName,configurationContext,policyName);
		PolicyRequestContext policyRequestContext = new PolicyRequestContext(configurationContext.getTenantId(),
				configurationContext.getSiteId(), configurationContext.getFeatureGroup(),
				configurationContext.getFeatureName(), configurationContext.getImplementationName(),
				configurationContext.getVendorName(), configurationContext.getVersion());
		...
		try {
			...
			if (psconfigUnit == null) {
				logger.warn("{} Delete request for Non Cache PolicyConfig= {} " ,LEAP_LOG_KEY, policyName);
				// delete from DB
				Integer configNodeId = getApplicableNodeId(policyRequestContext);
				boolean deletePolicyFromDb = deletePolicyFromDb(policyName, configNodeId);
				logger.debug("{} exiting from the {}", LEAP_LOG_KEY, methodName);
				return deletePolicyFromDb;
			}

			// Delete from the DB First so that configVerifier should not revitalise the
			// config in dataGrid
			IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
			...
			logger.debug("{} deleted from db NodeDataId= {} ",LEAP_LOG_KEY, psconfigUnit.getDbconfigId());

			// Now remove from DataGrid
			LeapConfigurationServer configServer = LeapConfigurationServer.getConfigurationService();
			configServer.deleteConfiguration(psconfigUnit.getTenantId(), psGroupKey, policyName);
			logger.debug("{} deleted from DataGrid psGroupKey={}, policyName={} " ,LEAP_LOG_KEY, psGroupKey, policyName);
	...
	private boolean deletePolicyFromDb(String configName, int nodeId) throws PolicyConfigurationException {
		// Delete from the DB
		IConfigPersistenceService configPersistenceService = new ConfigPersistenceServiceMySqlImpl();
		logger.debug("{} .deletePolicyFromDb() deleted from db configName= {} ",LEAP_LOG_KEY, configName);
		try {
	...
	/**
	 * based on the PolicyRequestContext and policy name searching Policy ,if policy
	 * Exist in DB check wether it enabled or not , if enabled check exist in cache
	 * or not , if not Exist load the data to cache and return true else false
	 * 
	...
	public boolean checkPolicyExistInDbAndCache(ConfigurationContext configurationContext, String policyName)
		...
		String methodName = "checkPolicyExistInDbAndCache";
		logger.debug("{} entered into the method {} with configurationContext={} , policyname={} ", LEAP_LOG_KEY, methodName,configurationContext,policyName);
	...
	public boolean reloadPolicyCacheObject(RequestContext requestContext, String policyName)
			throws PolicyConfigurationException {
		String methodName = "reloadPolicyCacheObject";
		logger.debug("{} entered into the method {}", LEAP_LOG_KEY, methodName);
		...
		try {
			...
			PolicyConfigurationUnit policyConfigurationUnit = getPolicyConfigurationUnit(policyRequestContext,
					policyName);
			if (policyConfigurationUnit == null) {
				...
				PolicyConfigurationUnit policyConfigUnit = new PolicyConfigurationUnit(requestContext.getTenantId(),
						requestContext.getSiteId(), configNodeData.getParentConfigNodeId(), true, policyConfig, null);
				policyConfigUnit.setDbconfigId(configNodeData.getNodeDataId());
				configUnitBuilder.buildPolicyConfigUnit(policyConfig, policyConfigUnit);
			...
			logger.error("{} Failed to reLoad policy from DB with Name= {} ",LEAP_LOG_KEY, policyName, e);
			...
			logger.error("{} Failed to xml-parse policy from DB with Name {} " ,LEAP_LOG_KEY, policyName, e);
