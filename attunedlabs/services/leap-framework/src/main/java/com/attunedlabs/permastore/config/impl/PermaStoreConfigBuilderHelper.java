...
/**
 * Helper class for calling an handling the process of building the cache Object
 * 
 * @author bizruntime
 *
 */
public class PermaStoreConfigBuilderHelper {
	 ...
	 */
	public Serializable handleConfigurationBuilder(ConfigurationBuilder configBuilder)
			throws PermaStoreConfigurationBuilderException {
		String methodName = "handleConfigurationBuilder";
		logger.debug("{} entered into the method {}", LEAP_LOG_KEY, methodName);
		String psCacheBuilderType = configBuilder.getType().value();
		logger.trace("{} psCacheBuilderType : {}", LEAP_LOG_KEY, psCacheBuilderType);
		Serializable objToCache = null;
		if (psCacheBuilderType.equalsIgnoreCase(PermaStoreConfigurationConstant.CONFIGBUILDER_CUSTOM)) {
			logger.trace("{} psCacheBuilderType is custom type", LEAP_LOG_KEY);
			...
			logger.debug("{} OSGIEnvironmentHelper.isOSGIEnabled : {}", LEAP_LOG_KEY,
			...
			if (OSGIEnvironmentHelper.isOSGIEnabled) {
				...
				try {
					customBuilder = (IPermaStoreCustomCacheObjectBuilder) beanResolutionHelper.resolveBean(
							IPermaStoreCustomCacheObjectBuilder.class.getName(), customBuilderConfig.getBuilder());
				} catch (InvalidSyntaxException e) {
					throw new PermaStoreConfigurationBuilderException(
							"Unable to Load/instantiate PermastoreCustomBuilder=" + customBuilderConfig.getBuilder(),
							e);
				}
			} else {
				try {
					IBeanDependencyResolver beanResolver = BeanDependencyResolverFactory.getBeanDependencyResolver();
					customBuilder = (IPermaStoreCustomCacheObjectBuilder) beanResolver.getBeanInstance(
							IPermaStoreCustomCacheObjectBuilder.class, customBuilderConfig.getBuilder());
			...
			if (configBuilder.getSQLBuilder() == null || configBuilder.getSQLBuilder().getSQLQuery() == null
					|| configBuilder.getSQLBuilder().getSQLQuery().getValue() == null
					|| configBuilder.getSQLBuilder().getSQLQuery().getMappedClass() == null) {
				new PermaStoreConfigurationBuilderException(
						"Invalid SQLBuilder Configuration mapper or SQL is missing");
			}
			try {
				SQLCacheObjectBuilder sqlCacheBuilder = new SQLCacheObjectBuilder();
				objToCache = sqlCacheBuilder.loadDataForCache(configBuilder.getSQLBuilder().getSQLQuery().getValue(),
						configBuilder.getSQLBuilder().getSQLQuery().getMappedClass());
			...
			throw new PermaStoreConfigurationBuilderException(
					"UnKnown-PSConfigBuilderType for configType=" + psCacheBuilderType);
		}
