 ...
 * jms subscriber configuration and the retryPolicy.
 * 
...
public class JMSLeapDefaultRetryStrategy extends AbstractSubscriptionRetryStrategy {

	public JMSLeapDefaultRetryStrategy(String strategyConfig) throws ConfigurationValidationFailedException {
		super(strategyConfig);
		initializeRetryConfig(getRetryConfiguration());
	}

	/**
	 * initialize in jsonConfig for specific jms default strategy.
	 * 
	...
	private void initializeRetryConfig(JSONObject retryConfigurationJSON) {
		...
		SubscriptionRetryPolicy.getRetryQueueName(retryConfigurationJSON);
		SubscriptionRetryPolicy.getRetryConsumers(retryConfigurationJSON);
	...
	public List<String> getAllStrategyConfigurationKeys() {
		return Arrays.asList(new String[] { SubscriptionRetryPolicy.RETRY_COUNT_KEY,
				...
				SubscriptionRetryPolicy.TOP_MAX_RETRY_RECORDS_COUNT_KEY, SubscriptionRetryPolicy.RETRY_QUEUE_NAME,
				SubscriptionRetryPolicy.RETRY_CONSUMERS });
	}

	@Override
	public void preProcessing(Exchange exchange, Map<String, Object> metaData) throws Exception {
		String methodName = "preProcessing";
		log.debug("{} entered into the method {} of JMSLeapDefaultRetryStrategy", LEAP_LOG_KEY, methodName);
		log.debug("{} exiting from the {}", LEAP_LOG_KEY, methodName);
	}

	@Override
	public void onSuccess(Exchange exchange, Map<String, Object> metaData) throws Exception {
		String methodName = "onSuccess";
		log.debug("{} entered into the method {} of JMSLeapDefaultRetryStrategy", LEAP_LOG_KEY, methodName);
		jmsEventSubscriptionLogService.updateSubscriptionRecordStatus(exchange, metaData,
				EventSubscriptionTrackerConstants.STATUS_COMPLETE, null, this.getRetryConfiguration());
		log.debug("{} exiting from the {}", LEAP_LOG_KEY, methodName);
	}

	@Override
	public void onFailure(Exchange exchange, Map<String, Object> metaData, Exception exception) throws Exception {
		...
		if (eventSubscriptionTracker.getStatus() == null || eventSubscriptionTracker.getStatus().trim().isEmpty()
				|| !isRetryTriggered) {
			log.debug("{} record exists or not :: {}", LEAP_LOG_KEY,
					jmsEventSubscriptionLogService.recordIsNotAlreadyPresent(exchange, metaData));
			if (jmsEventSubscriptionLogService.recordIsNotAlreadyPresent(exchange, metaData)) {
				jmsEventSubscriptionLogService.addNewSubscriptionRecord(exchange, metaData);
			} else
				jmsEventSubscriptionLogService.updateSubscriptionRecordStatus(exchange, metaData,
