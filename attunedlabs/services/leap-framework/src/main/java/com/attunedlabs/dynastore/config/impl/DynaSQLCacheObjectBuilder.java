...
public class DynaSQLCacheObjectBuilder implements IDynaStoreCustomInitializer {
	...
	public Map<String, Serializable> initializeDynastoreWithData(DynastoreInitializer configBuilderConfig)
			throws ConfigDynaStoreInitializationException {
		String methodName = "initializeDynastoreWithData";
		...
		try {
			...
			throw new ConfigDynaStoreInitializationException(
					"SQLBuilder.. SQLException occured during reading ResultSet for for sql=" + sqlString, sqlExp);
		} finally {
			CoreDataBaseUtil.dbCleanup(con, ptst, rs);
		}
	}

	private Map<String, Serializable> mapData(ResultSet rs, String mapper, String mapKey)
			throws ConfigDynaStoreInitializationException {
		logger.debug("{} Mapper IS {}", LEAP_LOG_KEY, mapper);
	...
	private Map<String, Serializable> mapResultSetToMap(ResultSet rs, String key)
			throws ConfigDynaStoreInitializationException {
			...
			if (rs != null) {
				...
				while (rs.next()) {
					...
					for (int i = 0; i < numberOfColumns; i++) {
						logger.trace("{} resultSet Index is {}", LEAP_LOG_KEY, (i + 1));
				...
				logger.debug("{} generatedListOfMap is {}", LEAP_LOG_KEY, dataMap);
				return dataMap;
			} // end of if
	...
	private String getMapKeyValue(ResultSet rs, String key)
			throws SQLException, ConfigDynaStoreInitializationException {
		Object keyValue = rs.getObject(key);
		if (keyValue == null)
			throw new ConfigDynaStoreInitializationException(
					"SQLBuilder.. key{" + key + "} for map in configuration is not found in the resultset");
		return keyValue.toString();
	}

	private Map<String, Serializable> customMapper(ResultSet resultSet, String mapperfqcn)
			throws ConfigDynaStoreInitializationException {
		...
		try {
			...
			logger.error("SQLBuilder.. SQLException occured during reading ResultSet from Mapper {" + mapperfqcn + "}",
					e);
			throw new ConfigDynaStoreInitializationException(
