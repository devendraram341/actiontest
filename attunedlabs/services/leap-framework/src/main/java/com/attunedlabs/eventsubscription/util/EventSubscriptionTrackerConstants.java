...
public class EventSubscriptionTrackerConstants {
	...
	private static Integer getRetryCountForFailedSubscription() {
		...
		try {
			...
			String retryCount = LeapConfigUtil.getGlobalPropertyValue(RETRY_COUNT_COL,LeapDefaultConstants.DEFAULT_DEFAULT_RETRY_COUNT);
			if (retryCount != null)
				return Integer.parseInt(retryCount);
		} catch (Exception e) {
			log.error(
					"{} failed to load retry count from consumer properties will use default value for retry count...{}",
					LEAP_LOG_KEY, SubscriptionConstant.KAFKA_CONSUMER_CONFIGS);
		}
